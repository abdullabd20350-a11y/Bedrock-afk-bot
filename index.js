...
app.get('/', checkAuth, (req, res) => {
    ...
    let botCards = myBots.map(id => {
        ...
        let statusText = b.connecting ? 'Connecting...' : (b.connected ? 'Online' : 'Offline');
        ...
        <div class="bot-card" style="border-${isAr?'right':'left'}: 6px solid ${b.connected?'#28a745':'#dc3545'};">
            ...
            <div id="edit-${id}" class="edit-panel">
                <form action="/edit" method="POST" style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                    ...
                    <button class="btn btn-start" style="grid-column:span 2;">Save Changes</button>
                </form>
            </div>
    ...
    <div class="card">
        <div style="display:flex; justify-content:space-between; align-items:center;"><h2>🚀 Kinga Dashboard</h2><a href="/logout" style="color:red; text-decoration:none;">Logout</a></div>
        ...
        <div id="botList">${botCards || '<p style="text-align:center; color:#999;">No bots yet</p>'}</div>
    </div>
    <script>
...
app.post('/control', checkAuth, (req, res) => {
    ...
    if (action === 'start' && !b.connected) {
        ...
        if (b.type === 'bedrock') {
            ...
            b.client.on('spawn', () => { b.connected = true; b.connecting = false; saveData(); });
            b.client.on('error', () => { b.connected = false; b.connecting = false; });
        } else {
            // العودة لنسخة الاتصال الأصلية والناجحة 100%
            b.client = mineflayer.createBot({
                host: b.host,
                port: b.port,
                username: b.botName
            });
            b.client.on('spawn', () => { b.connected = true; b.connecting = false; b.pos = b.client.entity.position; saveData(); });
            b.client.on('error', (err) => { console.log('Java Bot Error:', err.message); b.connected = false; b.connecting = false; });
            b.client.on('end', () => { b.connected = false; b.connecting = false; });
        }
    } else if (action === 'stop') {
