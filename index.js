...
// 🟢 الذاكرة الدائمة: وضعنا بوتك هنا لكي لا يحذفه Render أبداً عند إيقافه
let botsData = {
    "my_main_bot": { 
        id: "my_main_bot", 
        username: "bgrt5", 
...
let botClients = {}; // الذاكرة المؤقتة لجلسات الاتصال

// دالة تحميل البيانات (تدمج الذاكرة الدائمة مع الملف إن وُجد)
function loadData() {
    if (fs.existsSync(DATA_FILE)) {
        try {
            const raw = fs.readFileSync(DATA_FILE);
            const parsed = JSON.parse(raw);
            // دمج البوتات الإضافية مع البوت الأساسي
            botsData = { ...botsData, ...parsed };
        } catch (e) { console.error("خطأ في قراءة الملف."); }
    }
    
    // تصفير الحالات عند إعادة تشغيل السيرفر
...
// دالة حفظ البيانات للبوتات الإضافية
function saveData() {
...
function startBot(id) {
    ...
    // تنظيف تام لأي جلسة معلقة قبل البدء لتجنب تكرار البوتات
    if (botClients[id]) {
        try { botClients[id].disconnect(); } catch (e) {}
        delete botClients[id];
    }

    bot.status = 'جاري الاتصال المباشر...';
    bot.shouldBeRunning = true;
    saveData();

    try {
        console.log(`[${bot.username}] محاولة الدخول للسيرفر...`);
        
        const client = bedrock.createClient({
            host: bot.host,
            port: parseInt(bot.port) || 19132,
            username: bot.username,
            offline: true, 
            version: '1.26.12',
            deviceOS: 1,       // تمويه: البوت يلعب من هاتف أندرويد
            skipPing: true,    // تخطي رسائل الـ Ping لتجنب الحظر الفوري (Ping timed out)
        ...
        client.on('join', () => {
            ...
            console.log(`[${bot.username}] نجح في الدخول!`);
        });

        client.on('start_game', (pkt) => {
            const pos = pkt.player_position;
            // تجاهل الإحداثيات الوهمية التي يرسلها السيرفر في البداية
            if (pos && Math.abs(pos.y) < 30000) {
                bot.coordinates = `X: ${Math.floor(pos.x)}, Y: ${Math.floor(pos.y)}, Z: ${Math.floor(pos.z)}`;
            }
        });

        client.on('error', (err) => {
            console.log(`[خطأ - ${bot.username}]: ${err.message}`);
            bot.status = 'فشل (إعادة محاولة...)';
            bot.connectedAt = null;
        });

        client.on('close', () => {
            bot.coordinates = 'غير متوفر';
            bot.connectedAt = null;
            
            // إعادة الاتصال اللانهائي طالما زر الإيقاف لم يُضغط
            if (bot.shouldBeRunning) {
                console.log(`[${bot.username}] انقطع الاتصال. سيعود بعد 10 ثوانٍ...`);
                bot.status = 'إعادة اتصال...';
                delete botClients[id];
        ...
        console.log(`[انهيار - ${bot.username}]: ${e.message}`);
        setTimeout(() => {
            if (botsData[id] && botsData[id].shouldBeRunning) startBot(id);
        }, 10000);
    }
...
// 🛑 محرك الإيقاف (تم إصلاح مشكلة الحذف)
function stopBot(id) {
    if (botsData[id]) {
...
app.post('/api/bots', (req, res) => {
    ...
    botsData[id] = { id, username, host, port: parseInt(port) || 19132, status: 'مضاف', coordinates: 'غير متوفر', connectedAt: null, shouldBeRunning: false };
    saveData();
...
app.delete('/api/bots/:id', (req, res) => {
    ...
    // نمنع حذف البوت الأساسي المحفوظ في الذاكرة لتجنب أي مشاكل
    if (id === "my_main_bot") {
        return res.status(400).json({ error: "لا يمكن حذف البوت الأساسي" });
    }
    stopBot(id);
    delete botsData[id];
    saveData();
...
app.listen(PORT, () => { 
    loadData(); 
    console.log(`[النظام] خادم مدير البوتات يعمل بنجاح على ${PORT}`); 
});const express = require('express');
const bedrock = require('bedrock-protocol');
const fs = require('fs');
const path = require('path');
