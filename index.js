...
function startBotLogic(id) {
    ...
    client.on('spawn', () => {
        ...
        b.moveInterval = setInterval(() => {
            if (!b.connected || !client) return clearInterval(b.moveInterval);
            try {
                let p = { ...b.pos };
                if (Math.random() > 0.5) {
                    p.y += 1.2; 
    ...
    client.on('error', (err) => { b.connected = false; console.log(`Error: ${err.message}`); });
    client.on('close', () => { b.connected = false; });
}

// ==========================================
// 3. الواجهة (HTML الموقع)
// ==========================================
const ui = (content) => `
...
<style>
    ...
    .btn { padding: 8px 15px; border: none; border-radius: 8px; cursor: pointer; font-weight: bold; margin: 2px; }
    .btn-start { background: #28a745; color: white; }
    .btn-stop { background: #ffc107; color: #222; }
    .btn-del { background: #dc3545; color: white; }
    input { padding: 10px; border: 1px solid #ddd; border-radius: 8px; margin: 5px; width: 100%; max-width: 180px; }
    .xyz { background: #333; color: #0f0; padding: 8px 12px; border-radius: 8px; font-family: monospace; font-size: 0.9em; }
...
app.get('/', (req, res) => {
    ...
    res.send(ui(`
        ...
        <form action="/add" method="POST" style="background:#eee; padding:15px; border-radius:15px; margin-bottom:20px;">
            ...
            <button class="btn btn-start" style="width:100px;">إضافة</button>
        </form>
        ${botList || '<p>لا توجد بوتات مضافة</p>'}
    `));
});
